...
namespace PathologicalGames
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        public void Add(Transform instance, string prefabName, bool despawn, bool parent)
        {
            for (int i = 0; i < this._prefabPools.Count; i++)
            {
                ...
                if (this._prefabPools[i].prefabGO.name == prefabName)
                {
                    ...
                    if (parent) instance.SetParent(this.group); //instance.parent = this.group;
        ...
        {
            instance.SetParent(parent); //instance.parent = parent;
    ...
    public class PrefabPool
    {
        ...
        /// 脚本类型
        /// </summary>
        public System.Type scriptType = null; 
        ...
        internal void SelfDestruct()
        {
            ...
            foreach (Transform inst in this._spawned) {
                if (inst != null) {                    
                    this.spawnPool._spawned.Remove(inst);
                    Object.Destroy(inst.gameObject);
                }
            }
        ...
        public Transform SpawnNew(Vector3 pos, Quaternion rot)
        {
            ...
            //增加控制脚本
            CPoolPrefabInsBase script = inst.gameObject.AddComponent(this.scriptType) as CPoolPrefabInsBase;
            script.pool = this.spawnPool;

            this.nameInstance(inst);  // Adds the number to the end

            if (!this.spawnPool.dontReparent)
                inst.SetParent(this.spawnPool.group); //inst.parent = this.spawnPool.group;  // The group is the parent by default
